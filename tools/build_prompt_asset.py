...
def text_definition(template, character, width, height):
    # Retain the example's complete text-field format; only enlarge its bounds.
...
def build(source, destination):
    ...
    button_placement = next(body for code, body in tags(overlay, 4)
                            if code == 26 and b'test_button\0' in body)
    button_id, = struct.unpack_from('<H', button_placement, 3)
    ...
    text, prompt = range(next_id, next_id + 2)
    definitions = text_definition(template, text, 80000, 3000)

    # Frame 0 is genuinely empty; frame 1 creates the entire dialog at once.
    panel = bytearray(struct.pack('<HH', prompt, 3) + tag(1))
    # Reuse only the original MewUI button artwork that the game already loads.
    panel += place(button_id, 2, 'edge', 160, 5, 2.15)
    ...
    for i, (name, x) in enumerate((('prev', 218), ('next', 418), ('cancel', 670), ('yes', 870))):
        panel += place(button_id, 40 + i, name, x, 606, .42)
    ...
    panel += place(button_id, 2, 'edge', 220, 180, 1.85)
    panel += place(text, 5, 'title', 266, 239, .28)
    for i in range(4):
        panel += place(text, 10 + i, f'line{i}', 266, 285 + i * 26, .20)
    panel += place(button_id, 42, 'cancel', 830, 410, .42)
    ...
    for code, body in source_tags:
        if code == 82:
            ...
            body = (struct.pack('<H', count + 1) + body[2:] +
                    struct.pack('<H', prompt) + b'house_fla.CdfPrompt\0')
    ...
    assert b'house_fla.CdfPrompt\0' in output
